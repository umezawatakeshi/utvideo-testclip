...
my $raws = [
	...
	{ srcn => "yuv10bit",  ofmt => VDVFMT_P210, dstn => "yuv10bit_p210",  sizes => $sizes_422 },
	{ srcn => "yuv10bit",  ofmt => VDVFMT_P216, dstn => "yuv10bit_p216",  sizes => $sizes_422 },
	{ srcn => "yuv10bit",  ofmt => VDVFMT_YUV422P16LE, dstn => "yuv10bit_yuv422p16le",  sizes => $sizes_422 },
	{ srcn => "yuv10bit",  ofmt => VDVFMT_P010, dstn => "yuv10bit_p010",  sizes => $sizes_420 },
	{ srcn => "yuv10bit",  ofmt => VDVFMT_P016, dstn => "yuv10bit_p016",  sizes => $sizes_420 },
	{ srcn => "yuv10bit",  ofmt => VDVFMT_YUV420P16LE, dstn => "yuv10bit_yuv420p16le",  sizes => $sizes_420 },
];
...
VirtualDub.SaveAVI("clip000-$fourcc-$progint-$pred-div$div-$size.avi");
__EOT__
				}
			}
		}
...
VirtualDub.video.SetCompression("$fourcc", 0, 0, 0);
VirtualDub.video.SetCompData(4, "$confb64");
VirtualDub.SaveAVI("clip000-$fourcc-$pred-div$div-$size.avi");
__EOT__
			}
		}
	}
...
VirtualDub.video.SetCompression("$fourcc", 0, 0, 0);
VirtualDub.video.SetCompData(4, "$confb64");
VirtualDub.SaveAVI("clip000-$fourcc-div$div-$size.avi");
__EOT__
		}
	}
}
